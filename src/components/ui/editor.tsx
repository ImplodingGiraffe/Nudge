...
const editorContainerVariants = cva(
  'relative w-full cursor-text select-text overflow-y-auto caret-primary selection:bg-brand/[.13] selection:text-inherit focus-visible:outline-none [&_.slate-selection-area]:z-50 [&_.slate-selection-area]:border [&_.slate-selection-area]:border-brand/25 [&_.slate-selection-area]:bg-brand/15',
  {
    defaultVariants: {
      variant: 'default',
    },
...
export function EditorContainer({
  ...
  React.useEffect(() => {
    const clearPointerState = () => setIsPointerDown(false);
  ...
  return (
    <PlateContainer
      className={cn(
        ...
        isPointerDown && 'editor-text-selecting',
        className
      )}
      onPointerDownCapture={(event) => {
        ...
        if (
          ...
          event.button !== 0 ||
          (event.target as HTMLElement).closest('.slate-blockToolbar')
        ) {
          return;
        }

        if ((event.target as HTMLElement).closest('[data-slate-editor]')) {
