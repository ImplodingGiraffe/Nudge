...
const BlockCommentContent = ({ children, element }: PlateElementProps) => {
  ...
  const suggestionNodes = isTopLevelBlock
    ...
    : [];
  const { reviewedDiscussions, resolvedSuggestions } =
    useBlockDiscussionItems(blockPath);

  const suggestionsCount = resolvedSuggestions.length;
  const discussionsCount = reviewedDiscussions.length;
  ...
  const activeDiscussion =
    activeCommentId &&
    reviewedDiscussions.find((d) => d.id === activeCommentId);
  ...
  const selected =
    reviewedDiscussions.some((d) => d.id === activeCommentId) ||
  ...
  if (suggestionsCount + reviewedDiscussions.length === 0 && !draftCommentNode)
