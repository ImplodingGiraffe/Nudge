...
import { ElementApi, type TNode, KEYS, nanoid, NodeApi, TextApi } from 'platejs';
import { type PlateEditor, useEditorRef, usePluginOption } from 'platejs/react';

import { aiChatPlugin } from '@/components/editor/plugins/ai-kit';
import { readKey } from '@/lib/ai/config';

import { discussionPlugin } from './plugins/discussion-kit';
import { BaseAIPlugin, withAIBatch } from '@platejs/ai';
import { createGeminiChatStream } from '@/lib/ai/plate-gemini';
...
function createChatTransport({
  ...
  return new DefaultChatTransport({
    ...
    fetch: (async (_input, init) => {
      ...
      const editorToolName = editor.getOption(AIChatPlugin, 'toolName');
      const isCommentTool =
        editorToolName === 'comment' ||
        body.toolName === 'comment' ||
        body.ctx?.toolName === 'comment';
  ...
  const baseChat = useBaseChat<ChatMessage>({
    ...
    onData(data) {
      ...
      if (data.type === 'data-comment' && data.data) {
        ...
        if (aiComment) {
          let range = aiCommentToRange(editor, aiComment);

          if (!range) {
            try {
              const blockEntry = editor.api.node({
                at: [],
                match: (node) =>
                  ElementApi.isElement(node) &&
                  (node as any).id === aiComment.blockId,
              });

              if (blockEntry) {
                const [blockNode, blockPath] = blockEntry;
                const foundRange = findTextRangeInBlock({
                  block: blockEntry,
                  findText: aiComment.content,
                });
                if (foundRange) range = foundRange;

                if (!range) {
                  const blockStart = editor.api.start(blockPath);
                  const blockEnd = editor.api.end(blockPath);
                  if (blockStart && blockEnd) {
                    range = { anchor: blockStart, focus: blockEnd };
                  }
                }
              }
            } catch (error) {
              console.warn('Fallback range computation error:', error);
            }
          }
...
const createCommentChunks = (editor: PlateEditor) => {
  ...
  const chunks = indexes
    .map((index, i) => {
      ...
      texts: `{"id":"${nanoid()}","data":{"comment":{"blockId":"${block.id}","comment":"${faker.lorem.sentence()}","content":"${content}"},"status":"streaming"},"type":"data-comment"}`,
        },
      ];
    })
  ...
  const chunks = cellIds.map((cellId, i) => [
    {
      delay: faker.number.int({ max: 300, min: 100 }),
          texts: `{"id":"${nanoid()}","data":{"cellUpdate":{"id":"${cellId}","content":"${faker.lorem.sentence()}"},"status":"streaming"},"type":"data-table"}`,
    },
  ]);
