...
export const buildBlockDiscussionIndex = ({
  ...
  discussions.forEach((discussion) => {
    ...
    if (!ownerPath || !commentIds.has(discussion.id) || discussion.isReviewed) {
      return;
    }
...
const getDiscussionIndex = (
  ...
  const hasDiscussions = Boolean(discussions && discussions.length > 0);
  const activeSuggestionId = editor.getOption(suggestionPlugin, 'activeId');
  const activeCommentId = editor.getOption(commentPlugin, 'activeId');

  if (!hasDiscussions && !activeSuggestionId && !activeCommentId) {
    const hasAnyCommentsOrSuggestions = editor.api.some({
      match: (n: any) =>
        Boolean(n.comment || n.suggestion || n.suggestionId || n[getSuggestionKey()]),
    });

    if (!hasAnyCommentsOrSuggestions) {
      const emptyIndex: BlockDiscussionIndex = {
        discussionsByBlock: new Map(),
        suggestionsByBlock: new Map(),
      };
      discussionIndexCache.set(editor, { discussions, index: emptyIndex, version });
      return emptyIndex;
    }
  }
...
export const useBlockDiscussionItems = (blockPath: Path) => {
  ...
  return React.useMemo(() => {
    ...
    return {
      reviewedDiscussions: index.discussionsByBlock.get(blockKey) ?? [],
