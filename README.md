...
While this is awesome for privacy and offline use, it means Nudge doesn't support automatic syncing between devices. I decided to go this route because I wanted Nudge to be something _you_ own. Also, static websites are wonderfully simple and keeping it static means I can host the live version for free on Neocities. In the future I might look into adding an optional field in settings where you can drop in an API key for a cloud storage provider. This also has the benefit of keeping important data, like notes, safer from accidental deletion.
...
That said, I am wary of taking lecture notes here instead of, say, Word since everything lives in browser storage. If your browser data gets cleared out and you haven't exported a backup, your notes go down with the ship. I realize that this is a rather large issue and have considered the File System Access API to read and write note files locally on your computer. But this is only supported on Chromium-based browsers. (I'm typing this on Firefox by the way.) 🥲
